...
history_callback = model.fit(x=train_data[:,:9], y=train_data[:,9][:,np.newaxis], sample_weight=train_data[:,10][:,np.newaxis], validation_data=(val_data[:,:9], val_data[:,9], val_data[:,10]), batch_size=64, callbacks=[model_checkpoint_callback, early_stopping_callback], epochs=100, verbose=2)
